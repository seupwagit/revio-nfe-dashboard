...
export default function BuscaNaturalSimples({ onSearch, onClear }: BuscaNaturalSimplesProps) {
  ...
  const processarQueryComLLM = async (texto: string): Promise<{ filtros: any, explicacao: string }> => {
    ...
    try {
      setCarregando(true)
      console.log('🤖 Processando com LLM:', texto)
...
11. Retorne APENAS um objeto JSON válido, sem explicações ou texto adicional
...
AGORA PROCESSE A CONSULTA E RETORNE APENAS O JSON:`
  ...
  const processarQuery = (texto: string) => {
    ...
    const destinatario = textoLower.match(/(?:destinatário|destinatario|cliente)(?:\s+contém|\s+com)?\s+([a-záàâãéèêíïóôõöúçñ\s]+?)(?:\s+e\s+|\s+ou\s+|$)/i)
    if (destinatario) {
