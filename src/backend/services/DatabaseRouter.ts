...
export class DatabaseRouter {
  ...
  private async createSqlConnectionWithRetry(bancoDeDados: string, attempt: number = 0): Promise<PrismaClient | null> {
    ...
    try {
      ...
      // Substituir o database na connection string
      const dynamicUrl = baseUrl.replace(/database=([^;]+)/, `database=${bancoDeDados}`)
  ...
  setUserContext(context: UserContext): void {
    ...
    this.logConnectionEvent('context_set', context.bancoDeDados, context.usrCodigo)
    console.log(`[DatabaseRouter] Contexto definido para usuário: ${context.usrNome} -> ${context.bancoDeDados}`)
  }
  ...
  clearUserContext(): void {
    if (this.currentUserContext) {
      ...
      this.logConnectionEvent('context_cleared', context.bancoDeDados, context.usrCodigo)
      console.log(`[DatabaseRouter] Contexto limpo para usuário: ${context.usrNome}`)
   ...
   * Usa AsyncLocalStorage ou thread-local storage quando disponível
   */
  private getCurrentContextFromManager(): UserContext | null {
    try {
      // Tentar obter do contexto local primeiro
      if (this.currentUserContext) {
        return this.currentUserContext
      }

      // Para implementação futura com AsyncLocalStorage
      // Por enquanto, usar o contexto local definido pelo middleware
      return null
    } catch (error) {
  ...
  async switchUserContext(newContext: UserContext): Promise<void> {
    try {
      const oldContext = this.currentUserContext
  ...
  forceContextCleanup(reason: string = 'Limpeza forçada'): void {
    try {
      ...
      if (oldContext) {
        this.logConnectionEvent('context_cleared', oldContext.bancoDeDados, oldContext.usrCodigo, reason)
      }
