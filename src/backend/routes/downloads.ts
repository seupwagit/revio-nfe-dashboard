...
router.post('/schedule', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    // Validar formato das chaves (44 caracteres para chave NFe)
    const invalidChaves = chaves.filter((chave: any) => 
      typeof chave !== 'string' || chave.length !== 44
    )
    ...
    if (invalidChaves.length > 0) {
      ...
      return res.status(400).json({
        success: false,
        error: 'Algumas chaves possuem formato inválido (devem ter 44 caracteres)',
        code: 'INVALID_CHAVES'
      })
    }
