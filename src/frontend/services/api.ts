...
export async function fetchContador(filtros: Filtros): Promise<number> {
  try {
    ...
    const response = await httpService.get<CountResponse>(`/WebView/ContadorConsulta?${params.toString()}`)
    
    if (!response.success) {
      throw new Error(response.error || 'Erro na requisição do contador')
    }
