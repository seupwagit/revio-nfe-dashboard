...
export class AutoDownloadService {
  ...
  private async updateDownloadStatus(downloadId: number): Promise<void> {
    try {
      ...
      if (!response.success) {
        console.error(`[AutoDownload] ❌ Falha ao atualizar status do download ${downloadId}:`, response.error)
        throw new Error(response.error || 'Erro ao atualizar status no backend')
      }
