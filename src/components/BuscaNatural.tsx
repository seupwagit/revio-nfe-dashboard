...
export default function BuscaNatural({ onSearch, onClear }: BuscaNaturalProps) {
  ...
  const [reconhecimentoSuportado, setReconhecimentoSuportado] = useState(false)

  // Verificar se o navegador suporta reconhecimento de voz
  useEffect(() => {
    const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition
    setReconhecimentoSuportado(!!SpeechRecognition)
  }, [])
  ...
  const exemplos = [
    ...
    // ✅ TESTADO - Valores
    { texto: 'acima de 5000', descricao: 'Valor > R$ 5.000' },
    { texto: 'abaixo de 1000', descricao: 'Valor < R$ 1.000' },
    { texto: 'entre 1000 e 5000', descricao: 'Faixa de valores' },
    { texto: 'acima de mil', descricao: 'Valor > R$ 1.000 (por extenso)' },
    
    // ✅ TESTADO - Empresas (busca parcial)
    { texto: 'ciano', descricao: 'Busca "ciano" em emitente ou destinatário' },
    { texto: 'infoco', descricao: 'Busca "infoco" em razão social' },
    { texto: 'emitente ciano', descricao: 'Busca apenas em emitente' },
    { texto: 'destinatário vale', descricao: 'Busca apenas em destinatário' },
    
    // ✅ TESTADO - Status
    { texto: 'autorizadas', descricao: 'Notas autorizadas' },
    { texto: 'canceladas', descricao: 'Notas canceladas' },
    { texto: 'processando', descricao: 'Notas em processamento' },
    
    // ✅ TESTADO - Impostos
    { texto: 'icms maior que 100', descricao: 'ICMS > R$ 100' },
    { texto: 'frete maior que 50', descricao: 'Frete > R$ 50' },
    
    // ✅ TESTADO - Localização
    { texto: 'sp', descricao: 'Estado de São Paulo' },
    { texto: 'município são paulo', descricao: 'Município específico' },
    
    // ✅ TESTADO - Identificação
    { texto: 'série 1', descricao: 'Série da nota' },
    { texto: 'modelo 55', descricao: 'Modelo 55 (NF-e)' },
    
    // ✅ TESTADO - Combinações
    { texto: 'entrada acima de 5000', descricao: 'Entradas > R$ 5.000' },
    { texto: 'saída sp', descricao: 'Saídas de São Paulo' },
    { texto: 'ciano autorizadas', descricao: 'Notas da CIANO autorizadas' },
  ]
  ...
  const processarBusca = (texto: string) => {
    ...
    // Formato SIMPLES: "01/12/2025" ou "data 01/12/2025" ou "data emissao 01/12/2025" (busca exata nessa data)
    const dataSimples = textoOriginal.match(/(?:(?:data\s+)?(?:emiss[aã]o\s+)?)?(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})(?!\s*e\s*\d)/i)
    if (dataSimples && !textoOriginal.match(/(?:maior|menor|acima|abaixo|antes|depois|ap[oó]s|entre)/i)) {
      const dia = dataSimples[1].padStart(2, '0')
      const mes = dataSimples[2].padStart(2, '0')
      const ano = dataSimples[3]
      filtros.dataInicio = `${ano}-${mes}-${dia}`
      filtros.dataFim = `${ano}-${mes}-${dia}`
      explicacao += `Data Emissão = ${dia}/${mes}/${ano}. `
      
      console.log('🔍 BUSCA NATURAL - Data detectada:', { 
        textoOriginal, 
        dia, 
        mes, 
        ano, 
      ...
      // REMOVER TODA a expressão de data do texto (incluindo "data" e "emissao")!
      textoLower = textoLower.replace(/(?:data\s+)?(?:emiss[aã]o\s+)?\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}/gi, '').trim()
      
      console.log('🧹 Texto após remover data:', textoLower)
    }
    
    // Formato: "data emissão maior que 27/11/2025" ou "após 27/11/2025"
    const dataRegex = /(?:data\s+emiss[aã]o|data|emiss[aã]o)?\s*(?:maior que|ap[oó]s|depois de|a partir de|>)\s*(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/i
    ...
    const dataAntesRegex = /(?:data\s+emiss[aã]o|data|emiss[aã]o)?\s*(?:menor que|antes de|at[eé]|<=?)\s*(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/i
    ...
    const natureza = textoLower.match(/natureza\s+(?:de\s+)?(?:opera[cç][aã]o\s+)?([a-záàâãéèêíïóôõöúçñ\s]+)/i)
    if (natureza) {
    ...
    const manifestacao = textoLower.match(/manifesta[cç][aã]o\s+([a-záàâãéèêíïóôõöúçñ\s]+)/i)
    if (manifestacao) {
    ...
    // 10. Série e Modelo
    const serie = textoLower.match(/s[eé]rie\s+(\d+)/i)
    if (serie) {
    ...
    const emitente = textoLower.match(/(?:emitente|empresa|fornecedor|razão social|razao social)(?:\s+contém|\s+contem|\s+com|\s+contendo)?\s+([a-záàâãéèêíïóôõöúçñ\s\.]+?)(?:\s+e\s+|\s+ou\s+|$)/i)
    if (emitente) {
      filtros.emitente = emitente[1].trim()
      filtros.destinatario = emitente[1].trim() // Buscar em ambos
      explicacao += `Buscando "${filtros.emitente}" em emitente e destinatário. `
    }
    
    const destinatario = textoLower.match(/(?:destinatário|destinatario|cliente|comprador)(?:\s+contém|\s+contem|\s+com|\s+contendo)?\s+([a-záàâãéèêíïóôõöúçñ\s\.]+?)(?:\s+e\s+|\s+ou\s+|$)/i)
    if (destinatario) {
    ...
    if (!emitente && !destinatario && textoLimpoFinal.length >= 3 && !textoLower.match(/\b(entrada|saida|saídas|entradas|valor|icms|ipi|pis|cofins|frete|serie|modelo|cnpj|numero|nota|uf|municipio|dias|mes|ano|status|cancelada|autorizada|protocolada)\b/i)) {
      ...
      explicacao += `Buscando "${textoLimpoFinal}" em razão social (emitente ou destinatário). `
      console.log('🏢 Busca de empresa detectada:', textoLimpoFinal)
    }
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      // Processar automaticamente após reconhecer
      setTimeout(() => {
          ...
          <div className="absolute inset-y-0 right-0 flex items-center gap-1 pr-2">
            {reconhecimentoSuportado && (
              <button
                ...
                title={gravando ? 'Gravando... Fale agora!' : 'Busca por voz'}
              >
                {gravando ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
              </button>
            )}
            {query && (
              <button
