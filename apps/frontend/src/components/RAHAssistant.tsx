...
// import { rahAgent } from '../services/rahAgent' // Temporariamente desabilitado
...
export default function RAHAssistant() {
  ...
  const handleSend = async () => {
    ...
    try {
      // const response = await rahAgent.ask(input) // Temporariamente desabilitado
      const response = "RAH Agent temporariamente desabilitado"
      
      const assistantMessage: Message = {
        role: 'assistant',
        content: response, // Já é uma string
        timestamp: new Date()
      }
  ...
  const handleClear = () => {
    if (confirm('Limpar toda a conversa?')) {
      setMessages([])
      // rahAgent.clearHistory() // Temporariamente desabilitado
    }
  }
  ...
  // Sugestões de perguntas (ATUALIZADAS - Nova Arquitetura MongoDB)
  const suggestions = [
    "Como usar a busca natural?",
    "Como exportar dados para Excel?",
    "Como funciona a conexão direta com MongoDB?",
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all hover:scale-110 z-50"
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 h-[600px] bg-white rounded-lg shadow-2xl flex flex-col z-50 border-2 border-purple-200">
      {/* Header */}
