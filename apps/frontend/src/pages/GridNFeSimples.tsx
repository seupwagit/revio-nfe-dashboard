...
import FloatingManifestationButton from '../components/FloatingManifestationButton'
import GridPaginada from '../components/GridPaginada'
import LoadingSpinner from '../components/LoadingSpinner'
import { ManifestationTypeSelector } from '../components/ManifestationTypeSelector'
...
export default function GridNFeSimples() {
  ...
  // State for manifestation type (lifted for searchable select integration)
  const [selectedManifestationType, setSelectedManifestationType] = useState<string | null>(null)

  // Handle DANFE viewer open
  const handleVisualizarClick = (rowData: any) => {
    const documentId = rowData.id || rowData._id;
    console.log(`[GridNFeSimples] Opening DANFE viewer for document: ${documentId}`)
    setDanfeViewer({
      isOpen: true,
      documentId,
        ...
        return (
          <div className="flex items-center space-x-2">
            <button
              ...
              title={hasValidId ? 'Visualizar DANFE' : 'ID do documento não disponível'}
            >
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Botão de Manifestação Flutuante - Fica logo abaixo do download */}
      <FloatingManifestationButton
        selectedType={selectedManifestationType}
        onTypeChange={setSelectedManifestationType}
        stickyTop={true}
        topOffset={70}
      ...
      <GridPaginada
        ...
        columns={columns}
        pageSize={50}
        hideBusca={true} // ← Oculta a caixa de busca de documentos
        data-testid="grid-paginada"
      />
