...
import { ContextError, DatabaseContext, UserContext } from '../types/UserContext'
...
export class UserContextMiddleware {
  ...
  static authenticateAndSetupContext = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    // Envolver a execução da requisição em um contexto isolado
    return userContextManager.runWithContext(req.requestId, async () => {
      try {
        ...
        // Configurar contexto no DatabaseRouter para esta requisição (Ainda necessário para compatibilidade, mas agora o Router usará ALS)
        databaseRouter.setUserContext(userContext)

        // Anexar contexto à requisição para acesso direto
        req.userContext = userContext
        req.userDatabase = userContext.bancoDeDados
