...
import { ContextError, ContextMetrics, DatabaseContext, StoredContext } from '../types/UserContext';
...
export class UserContextManager {
  ...
  setContext(requestId: string, context: DatabaseContext): void {
    try {
      ...
      // Allow null userContext for global/unauthenticated contexts
      // This is valid for APIs that can work without authentication
      // No validation needed for userContext - null is explicitly allowed
      ...
      const storedContext: StoredContext = {
        requestId,
        userContext: context.userContext, // Allow null for global contexts
        createdAt: new Date(),
      ...
      if (context.userContext) {
        ...
        console.log(`[UserContextManager] Contexto global definido - Request: ${requestId}`)
      }
  ...
  /**
   * Obtém o ID da requisição atual do storage assíncrono
   */
  ...
  /**
   * Obtém o contexto do usuário atual (baseado no thread local / AsyncLocalStorage)
   */
  getCurrentContext(): DatabaseContext | null {
    const requestId = this.getCurrentRequestId()
    return requestId ? this.getContext(requestId) : null
  }
  ...
  listActiveContexts(): Array<{ requestId: string; user: string; database: string; age: number }> {
    ...
    for (const [requestId, context] of this.contextStore.entries()) {
      contexts.push({
        requestId,
        user: context.userContext?.usrNome || 'global',
        database: context.userContext?.bancoDeDados || 'spedrevio',
