...
async function testar90Dias() {
  console.log('🧪 Testando 90 dias com pageSize 10000...\n');

  const hoje = new Date();
  const inicio = new Date();
  inicio.setDate(hoje.getDate() - 90);

  console.log(`📅 Período: ${inicio.toISOString().split('T')[0]} até ${hoje.toISOString().split('T')[0]}`);
  console.log(`📦 PageSize: 10000\n`);
